...
from bct import INJECTION_TIME_MS, beam_injected, beam_alive
from tune_trim import set_tune_flat

studydesc = "Qx fixed, Qy stepped scan across the Qx=Qy coupling resonance, with full BBQ+BCT saving, damper OFF, chroma (0.55,0.55)"
shortname = "QY_CROSSING_SCAN_NO_DAMPER"  # SET: rename to something descriptive of this campaign
repetitions = 3
...
# --- Qx: fixed for the whole scan, set once, SET MANUALLY ---
_QX = 20.13
...
def name():
    return f"results_qy_crossing/{shortname}/QY{current_qy():.4f}/id{current_repetition()}"
...
def print_log(*args, **kwargs):
    print(*args, **kwargs)
    with open("run_qy_crossing_scan.log", "a") as fid:
...
def qy_scan_measurement(_name, data, header):
    ...
    print_log(f"Cycle {cycle_time}: Qx={_QX:.4f} Qy={current_qy():.4f} rep {current_repetition()}", end="")
...
japc.rbacLogin()
set_tune_flat(_QX, cycle=_CYCLE_NAME, plane='H', description=f"Qy crossing scan: Qx={_QX:.4f} (fixed)")
#subscribe_bbq_kicked_full(japc)
japc.subscribeParam(_BBQ, qy_scan_measurement, getHeader=True)

print_log()
print_log(f"{this_time}: Starting run with name '{shortname}', Qx={_QX:.4f} fixed, "
          f"Qy {qy_values[0]:.4f} -> {qy_values[-1]:.4f} (step {_QY_STEP}). Fire kicks manually.")
