...
def _store_data(name, result, header=None):
    name = name.split('+')[0]
...
def log_bbq(name, data, header):
   ...
   _store_data(f"rawdata/BBQ_data_{cycle_time}", data, header=header)

def log_bbq_full(name, data, header):
   cycle_time = header['cycleStamp']
   _store_data(f"rawdata/BBQ_full_data_{cycle_time}", data, header=header)
